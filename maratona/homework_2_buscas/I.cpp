...
int main(){
    ...
    cin >> num_days;
    moedas.resize(num_days);
    for (int day = 0; day < num_days; day++){
        cin >> moedas[day]; 
    }

    for (int moeda = moedas.size()-1; moeda >= 0; moeda--){
        auto maior_que_moeda = upper_bound(precos_lojas.begin(), precos_lojas.end(), moedas[moeda]);
        int num_possibles;

        if (maior_que_moeda != precos_lojas.end()){
            num_possibles = precos_lojas.size() - (maior_que_moeda - precos_lojas.begin());
        } else {
            num_possibles = 0;
        }
